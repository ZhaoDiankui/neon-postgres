	...
	else
	{
		...
		if (RelationNeedsWAL(index))
		{
			...
			SetLastWrittenLSNForBlockRange(XactLastRecEnd,
							  (RelationGetSmgr(index))->smgr_rlocator.locator,
...
gist_indexsortbuild(GISTBuildState *state)
{
	...
	if (RelationNeedsWAL(state->indexrel))
	{
		...
		lsn = log_newpage(&state->indexrel->rd_locator, MAIN_FORKNUM, GIST_ROOT_BLKNO,
 					levelstate->pages[0], true);
		SetLastWrittenLSNForBlock(lsn, state->indexrel->rd_locator,
								  MAIN_FORKNUM, GIST_ROOT_BLKNO);
		SetLastWrittenLSNForRelation(lsn, state->indexrel->rd_locator, MAIN_FORKNUM);
	}
