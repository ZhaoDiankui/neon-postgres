...
ginbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	 ...
	 */
	// FIXME Resolve merge conflict changes here
	if (RelationNeedsWAL(index))
	{
		log_newpage(&index->rd_locator, MAIN_FORKNUM,
					  0, BufferGetPage(MetaBuffer), true);

		SetLastWrittenLSNForBlockRange(XactLastRecEnd, (RelationGetSmgr(index))->smgr_rlocator.locator, MAIN_FORKNUM, 0, RelationGetNumberOfBlocks(index));
		SetLastWrittenLSNForRelation(XactLastRecEnd, (RelationGetSmgr(index))->smgr_rlocator.locator, MAIN_FORKNUM);
	}
